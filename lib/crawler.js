...
const getParams = () => {
  ...
  if (process.argv.length > 2) {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (param[0] === 'code') {
        const code = param[1].toString()
        codeObj.pCode = code.substr(0, 2)
        if (code.length > 2) {
          codeObj.cCode = code.substr(0, 4)
        }
      }
    }
  }
 ...
 */
exports.fetch = (host, route, regexp, codeLen, pCode = 0, cCode = 0) =>
  new Promise((resolve, reject) => http.get({
    ...
    res.on('end', () => {
      ...
      while ((current = regexp.exec(rawData)) !== null) {
        if (pCode && current[1].substr(0, codeLen) !== pCode) {
          continue
        } else if (cCode && current[1].substr(0, codeLen) !== cCode) {
          continue
        }
        if (current.length > 3) { // village
          if (parseInt(current[2].trim()) < 200) {
            continue
          } else {
            result[current[1].substr(0, codeLen)] = current[3].trim().replace(/居(.*?)委(.*?)会/, '').replace(/村(.*?)委(.*?)会/, '村').replace(/村委/, '村').replace(/管理委员会/, '')
          }
        } else {
          result[current[1].substr(0, codeLen)] = current[2].trim().replace(/居(.*?)委(.*?)会/, '').replace(/村(.*?)委(.*?)会/, '村').replace(/村委/, '村').replace(/管理委员会/, '')
        }
      }
...
exports.fetchCities = async (pCode) => {
  try {
    const codeObj = getParams()
    return await exports.fetch(host, pCode, casReg, 4, 0, codeObj.cCode)
  } catch (err) {

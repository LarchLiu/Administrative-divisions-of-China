    ...
    pd.children = _.map(p.cities, c => {
      ...
      cd.children = _.map(c.areas, a => {
        ...
        ad.children = _.map(a.streets, s => {
          ...
          const { code, name } = sd
          log(index, count, code, name, 5)
          sd.children = _.map(s.villages, v => {
            const vd = v.dataValues
            return _.pick(vd, cField)
          })
          if (sd.children.length !== 0) {
            r[pd.name][cd.name][ad.name][sd.name] = _.map(sd.children, v => v.name)
          } else {
            delete sd.children
          }
          return _.pick(sd, fField)
        })
        return _.pick(ad, fField)
      })
      return _.pick(cd, fField)
...
function log (index, total, code, name, type) {
  ...
  switch (type) {
    ...
    case 4:
      ...
      break
    case 5:
      text = '正在格式化省市区镇村五级联动数据，该步骤比较耗时，请耐心等候'
      break
  }
